...
function getSumToN(n) {
  ...
  for (let i = 0; i <= n; i += 1) {
    b.push(i);
    str = b;
    sum = str.reduce((acc, curr) => acc + curr, 0);
  }
  return sum;
}
...
function getSumOfDigits(n) {
  ...
  const result = str.map(Number);
  const done = result.reduce((acc, number) => acc + number, 0);
  return done;
}
...
function numberToStringInBase(number, base) {
  const leaveMeAlone = 0;
  if (typeof number === 'number') {
    return parseInt(String(number), leaveMeAlone).toString(base);
  }
  return parseInt(number.toString(), base).toString(base);
}
...
function getCountOfOddNumbers(number) {
  const numPlus = Math.abs(number);
  const a = [];
  let b = 0;
  for (let i = 0; i <= numPlus; i += 1) {
    a.push(i % 2);
  }
  b = a.reduce((acc, currentValue) => acc + currentValue, 0);
  return Number(b);
}
